...
class Robot:
    ...
    def drive(self, drive_meas):
        ...
        if angular_velocity == 0:
            ...
            # self.state[2] = (-0.01485446135524356467176248458143*self.state[2] +0.14167532096259588346666666666665+ 0.8699913457040707832)*self.state[2]
    ...
    def derivative_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            ...
            DFx[0,2] = (-np.cos(th)+np.cos(th+ang_vel*dt))*lin_vel/ang_vel
            DFx[1,2] = (-np.sin(th)+np.sin(th+ang_vel*dt))*lin_vel/ang_vel
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            Jac2[0,0] = np.cos(th)*dt #dx_dv
            Jac2[1,0] = np.sin(th)*dt #dy_dv
        else:
            Jac2[0,0] = (1/ang_vel)*(-np.sin(th) + np.sin(th2)) #dx_dv
            Jac2[0,1] = ((lin_vel/ang_vel)*np.cos(th2)*dt) - ((lin_vel/ang_vel**2)*(-np.sin(th)+np.sin(th2)))
            Jac2[1,0] = (1/ang_vel)*(np.cos(th) - np.cos(th2))
            Jac2[1,1] = ((lin_vel/ang_vel)*np.sin(th2)*dt) - ((lin_vel/ang_vel**2)*(np.cos(th)-np.cos(th2)))
            Jac2[2,1] = dt
